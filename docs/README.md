...
Complete documentation for hPanel web hosting control panel.
...
```bash
# One-command install
cd /opt && sudo git clone https://github.com/SefionITServices/clearPanel.git hpanel && cd hpanel && sudo ./install.sh
```
...
└───────┼──────────┼──────────┼──────────┼────────┘
        ...
        ↓          ↓          ↓          ↓
┌───────────┬─────────┬──────────┬───────────┐
│  Folder   │ dns.json│  BIND9   │   File    │
│ Structure │  (JSON) │  Zones   │  System   │
│           │         │          │           │
│ /Domains/ │ Tracker │ /etc/bind│  /home/   │
│ domain1/  │         │ /zones/  │  sefion/  │
│ domain2/  │         │          │           │
└───────────┴─────────┴──────────┴───────────┘
        │                  │
        ↓                  ↓
...
**Systemd Service:** `/etc/systemd/system/hpanel.service`
```ini
[Unit]
Description=hPanel Web Hosting Control Panel
After=network.target

[Service]
Type=simple
User=hpanel
WorkingDirectory=/opt/hpanel/backend
...
**Nginx Config:** `/etc/nginx/sites-available/hpanel`
```nginx
server {
    listen 80;
